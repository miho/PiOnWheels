...
public class ConicalGradient {
    ...
    private boolean             createRoundImage;
    ...
    public ConicalGradient(final Point2D CENTER, final boolean ROUND_IMAGE, final double OFFSET, final Stop... STOPS) {
        this(CENTER, ROUND_IMAGE, OFFSET, Arrays.asList(STOPS));
    }
    public ConicalGradient(final Point2D CENTER, final double OFFSET, final List<Stop> STOPS) {
        this(CENTER, false, OFFSET, STOPS);
    }
    public ConicalGradient(final Point2D CENTER, final boolean ROUND_IMAGE, final double OFFSET, final List<Stop> STOPS) {
    ...
    public Image getImage(final double WIDTH, final double HEIGHT) {
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                ...
                if (createRoundImage && (distance > width * 0.5 || distance > height * 0.5)) {
                    PIXEL_WRITER.setColor(x, y, Color.TRANSPARENT);
                } else {
